...
public class StandardLotteryService implements LotteryService {
	...
	public LotteryModel draw() {
		return new LotteryModel(ThreadLocalRandom.current().ints(1,lotteryMax).distinct().limit(lotterySize).sorted().boxed().toList());
	}

	@Override
	public List<LotteryModel> draw(int column) {
		return IntStream.range(0, column).mapToObj(_ -> this.draw()).toList();
	}
